...
#else
IPv6CommunicationSocket::IPv6CommunicationSocket(const IPv6Address &ipAddress, uint16_t ui_port, SOCKET client, unsigned int timeout) :
#endif //__linux__
	...
	counter(nullptr)
{
	...
	addr.sin6_port = uint16_t(htons(port));
